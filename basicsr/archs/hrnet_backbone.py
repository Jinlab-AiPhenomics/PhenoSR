...
class HighResolutionModule(nn.Module):
    """
    对每一个分辨率表示的分支进行特征提取。当只有一个分辨率分支时，就没有融合模块，直接返回结果。当有多个分支流的时候就需要先对各个分支进行计算，最后执行融合过程。
    """
    ...
    def _make_one_branch(self, branch_index, block, num_blocks, num_channels, stride=1):
        downsample = None
        # 判断是否通过1*1卷积降维，或者输入输出的通道是否一致，不一致使用1*1卷积进行维度升/降，后接BN，不使用ReLU
    ...
    # 循环调用上面说的_make_one_branch函数，比如并行三个分支的话，就要调用三次。
...
class HighResolutionNet_Classification(nn.Module):
    ...
    def forward(self, x):
        ...
        x_list = []
        for i in range(2):
            # transition层，将上一个stage
            if self.transition1[i] is not None:
                x_list.append(self.transition1[i](x))
            else:
                x_list.append(x)
        # 输入transition层得到的特征，进行特征融合得到特征融合后的输出，输入给第二个transition层得到stage3的输入特征，后面类推
...
if __name__ == "__main__":
    model = hrnet_classification(backbone='hrnetv2_w32')
    data = input = torch.rand(1, 3, 256, 256)
    model(data)
