...
@weighted_loss
def l1_loss(pred, target, stage="train", weight=None, reduction="mean"):
    if stage == "train":
        coarse, refine, _, current_iter = pred
        coarse_loss = F.l1_loss(coarse, target, reduction="none")
        refine_loss = F.l1_loss(refine, target, reduction="none")
        return coarse_loss + refine_loss
    elif stage == "finetune":
        coarse, refine, _, current_iter = pred
        coarse_loss = F.l1_loss(coarse, target, reduction="none")
        refine_loss = F.l1_loss(refine, target, reduction="none")
...
class L1Loss(nn.Module):
    ...
    def forward(self, pred, target, stage="train", weight=None, **kwargs):
        """
        Args:
        ...
        return self.loss_weight * l1_loss(
            pred, target, weight, stage=stage, reduction=self.reduction
        )
