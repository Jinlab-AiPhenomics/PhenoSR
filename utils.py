...
class Generator:
    ...
    def process(self, imgname):
        # model inference
        self.sr_output, self.seg_output, self.require_refine, self.se_score = (
            self.model(self.img, is_training=False)
        )
    ...
    def tile_process(self):
        ...
        self.sr_output = self.img.new_zeros(output_shape)
    ...
    def post_process(self):
        # remove extra pad
        if self.mod_scale is not None:
            _, _, h, w = self.sr_output.size()
            self.sr_output = self.sr_output[
                :,
                :,
    ...
    @torch.no_grad()
    def enhance(self, img, outscale=None, alpha_upsampler="realesrgan", imgname=None):
