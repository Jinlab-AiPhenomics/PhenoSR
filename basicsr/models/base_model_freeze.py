...
class BaseModel:
    ...
    def _initialize_best_metric_results(self, dataset_name):
        ...
        # # TODO：支持根据不同指标设置不同的patience
        # # 现在只支持miou或psnr中的一种
        # if 'patience' in self.opt['val']['metrics']['miou'].keys():
        #     self.best_metric_results['patience'] = self.opt['val']['metrics']['miou'].get('patience')
        # elif 'patience' in self.opt['val']['metrics']['psnr'].keys():
        #     self.best_metric_results['patience'] = self.opt['val']['metrics']['psnr'].get('patience')
        # else:
        #     self.best_metric_results['patience'] = None
        self.best_metric_results["patience"] = self.opt["val"].get("patience")
        self.best_metric_results["count_patience"] = 0
        self.best_metric_results["l_seg_val"] = 1
